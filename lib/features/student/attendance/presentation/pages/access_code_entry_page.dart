...
class _AccessCodeEntryPageState extends State<AccessCodeEntryPage> {
  ...
  void _submit() async {
    ...
    if (session == null) {
      setState(() { _error = 'No active session found'; _isSubmitting = false; });
      return;
    }
    
    if (!session.isActive) {
      setState(() { _error = 'Session expired'; _isSubmitting = false; });
      return;
    }
    ...
    if (code == session.accessCode) {
      ...
      setState(() { _error = 'Invalid code'; _isSubmitting = false; });
    }
  }
