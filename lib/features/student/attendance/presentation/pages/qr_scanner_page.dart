...
class _QRScannerPageState extends State<QRScannerPage> {
  ...
  void _processQr(String data) async {
    ...
    if (session == null) {
      setState(() { _error = 'No active session found'; _isProcessing = false; });
      return;
    }
    
    if (!session.isActive) {
      setState(() { _error = 'Session has expired'; _isProcessing = false; });
      return;
    }
    ...
    if (data == session.qrData) {
      ...
      setState(() { _error = 'Invalid QR code'; _isProcessing = false; });
    }
  }
          ...
          if (_error != null)
            Container(
              ...
              child: Row(children: [
                ...
                TextButton(onPressed: () => setState(() { _error = null; _isProcessing = false; }), child: const Text('Try Again')),
              ]),
            ),
        ],
      ),
